...
public class MateriAjarController {
    ...
    @PutMapping(path = "/put/{id}", consumes = "multipart/form-data")
    public ResponseEntity<CommonResponse<MateriAjar>> updateMateriAjar(@PathVariable("id") Long id, MateriAjar prestasi, @RequestPart("file") MultipartFile multipartFile) throws SQLException, ClassNotFoundException {
        CommonResponse<MateriAjar> response = new CommonResponse<>();
        try {
            MateriAjar tabelDip = materiAjarService.edit(prestasi, multipartFile, id);
            response.setStatus("success");
            response.setCode(HttpStatus.OK.value());
